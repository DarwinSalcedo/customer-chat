...
class Close(var chatHeads: ChatHeads) : View(chatHeads.context) {
    ...
    fun onPositionUpdate() {
        if (chatHeads.closeCaptured && chatHeads.topChatHead != null) {
            chatHeads.topChatHead!!.springX.endValue =
                springX.endValue + width / 2 - chatHeads.topChatHead!!.params.width / 2 + 2
            chatHeads.topChatHead!!.springY.endValue =
                springY.endValue + height / 2 - chatHeads.topChatHead!!.params.height / 2 + 2
        }
    }
    ...
    init {
        ...
        springScale.addListener(object : SimpleSpringListener() {
            override fun onSpringUpdate(spring: Spring) {
                //    bitmapBg =  Bitmap.createScaledBitmap(BitmapFactory.decodeResource(UIService.instance.resources, R.drawable.close_bg), (spring.currentValue + ChatHeads.CLOSE_SIZE).toInt(), (spring.currentValue + ChatHeads.CLOSE_SIZE).toInt(), false)
                invalidate()
            }
        })
